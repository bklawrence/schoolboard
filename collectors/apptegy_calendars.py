...
STM_SOURCE = "Saint Thomas More Calendar"
STM_CALENDAR = Unit4Calendar(
    "stm",
    "The High School of Saint Thomas More",
    "https://www.hs-stm.org/events",
)
...
def fetch_unit4_district_calendar(
    *,
    ...
    """Fetch Unit 4's public district calendar and apply it to all Unit 4 schools."""
...
def fetch_stm_calendar(
    *,
    timeout: int = 30,
    opener=urlopen,
) -> list[dict]:
    """Fetch Saint Thomas More's public Apptegy calendar."""
    events = fetch_unit4_calendar(
        STM_CALENDAR,
        timeout=timeout,
        opener=opener,
    ...
    for original in events:
        ...
        event["schools"] = ["stm"]
        event["scope"] = "school"
        event["source"] = STM_SOURCE
        event["sourceUrl"] = STM_CALENDAR.events_url
        normalized.append(event)
